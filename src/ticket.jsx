...
function ticket(props) {
    ...
    const Check_out = moment(props.data.date).add(parseInt(props.data.n_days), 'days').format('DD/MM/YYYY');
    const array_adult = Array.from({ length: props.data.p_adult / 500 }, (_, index) => index + 1);
    const array_child = Array.from({ length: props.data.p_child / 100 }, (_, index) => index + 1);

    console.log(props.data.p_child);
        ...
        <div className='tickets'>
            ...
            <main class="ticket-system">
                ...
                <div class="receipts-wrapper">
                    ...
                    {array_adult.map(a =>
                        <div id={a} class="receipts">
                            <div class="receipt">
                                <div class="route">
                                    <h5 style={{ marginBottom: "10px" }}>{props.data.hotel_name}</h5>
                                    <i class="fa-solid fa-hotel"></i>
                                </div>
                                <div class="details">
                                    <div class="item">
                                        <span style={{ fontWeight: "bold" }}>Check in</span>
                                        <p>{moment(props.data.date).format('DD/MM/YYYY')} <p>{props.data.time}</p></p>
                                    </div>
                                    <div class="item">
                                    ...
                                    <div class="item">
                                        <p style={{ display: "flex", gap: "10px", alignItems: "center" }}><i class="fa-solid fa-location-pin"></i><span>{props.data.vicinity}</span></p>
                                    </div>
                                    <div class="item">
                                        <div style={{ display: "flex", gap: "3px", marginTop: "10px" }}>
                                            <span style={{ marginRight: "5px", fontWeight: "bold" }}>Rating</span>
                                            {Array.from({ length: props.data.rating }).map((r) =>
                                                <span key={r}><i class="fa-regular fa-star"></i></span>
                                            )}
                                        </div>
                                    </div>
                                    {/* <div class="item">
                                        <div style={{ marginTop: "20px"}}>
                                            <p style={{ marginRight: "5px", marginBottom:"0", fontWeight: "bold" }}>Total : 500 DH</p>
                                        </div>
                                    </div> */}
                                </div>
                            </div>
                            <div class="receipt qr-code">
                                ...
                                <div class="description">
                                    <h5>500 DH</h5>
                                    <p style={{ fontSize: "11px" }}>Thank you, see you soon</p>
                                </div>
                            </div>
                        </div>
                    )
                    ...
                    {array_child.map(a =>
                        <div id={a} class="receipts">
                            ...
                            <div class="receipt qr-code">
                                ...
                                <QRCode
                                    value={`${props.data.id}-${props.data.name}-${props.data.hotel_name}`}
                                    size={100}
                                />
                                <div class="description">
                                    <h5>100 DH</h5>
